package ReviewCalss;

import java.util.Scanner;

public class Task3 {
public static void main(String[] args) {
//Write a program that prints out if it is good weather to go for a bike ride.
//The weather is good if the temperature is between 60 degrees and 100 degrees
//inclusive unless it is raining.
//If temperature is less than 60 program should say it is too cold, 
//if the temperature is more than 100, program should say it is too hot.
//If the temperature is between 40 degrees and 80 degrees inclusive & no rain--> we will go for hiking otherwise--> we will not go hiking
//If temperature is between 25 and 40 inclusive & snowing -->going snowboarding
//otherwise we are not going snowboarding it
//if temperature is more than 80 & sunny--> go to the beach otherwise --> not go to the beach*/

Scanner scan=new Scanner(System.in);
System.out.println("enter the tempreture");

boolean degree=scan.nextBoolean();

int degree;

	if (degree<25 && degree>40) {
		System.out.println("going snowboarding");
	}else {
		System.out.println("we are not going snowboarding");
	
	}if (degree<40 && degree>80) {
	System.out.println("we will go for hiking");
    }else {
	System.out.println("we will not go hiking");
    }if (degree<60) {
		System.out.println("too cold");
    }else{
    	System.out.println("too hot");
  }}}
